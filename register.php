...
<main class="sub-container">
  <form action="" method="post">
      <h3 class="card-header">お客様登録<span class="err-area"><?php if(!empty($err_msg['common'])){ echo $err_msg['common'];} ?></span></h3>
      
      <h4 class="sub-header">ログイン情報</h4>
      
      <h5 class="title"><span class="badge badge-req">必須</span>ログインID<span class="err-area"><?php if(!empty($err_msg['id'])){ echo $err_msg['id'];} ?></span></h5>
      ...
      <h5 class="title"><span class="badge badge-req">必須</span>パスワード<span class="err-area"><?php if(!empty($err_msg['pass'])){ echo $err_msg['pass'];} ?></span></h5>
      ...
      <input type="password" name="password-re" class="long-text" value="<?php if(!empty($pass_re)){ echo $pass_re;} ?>">
      <p class="validation">※確認のためにもう一度パスワードを入力してください。</p>
      
      <h4 class="sub-header">基本情報</h4>
      
      <h5 class="title"><span class="badge badge-req">必須</span>お名前<span class="err-area"><?php if(!empty($err_msg['name'])){ echo $err_msg['name'];} ?></span></h5>
      <input type="text" name="name" class="long-text <?PHP if(!empty($err_msg['name'])){ echo 'err';} ?>" value="<?php if(!empty($name)){ echo $name;} ?>">
      
      <h5 class="title"><span class="badge badge-req">必須</span>フリガナ<span class="err-area"><?php if(!empty($err_msg['slug'])){ echo $err_msg['slug'];} ?></span></h5>
      <input type="text" name="slug" class="long-text <?PHP if(!empty($err_msg['slug'])){ echo 'err';} ?>" value="<?php if(!empty($slug)){ echo $slug;} ?>">
      
      <h5 class="title"><span class="badge badge-req">必須</span>メールアドレス<span class="err-area"><?php if(!empty($err_msg['email'])){ echo $err_msg['email'];} ?></span></h5>
      <input type="text" name="email" class="long-text <?PHP if(!empty($err_msg['email'])){ echo 'err';} ?>" value="<?php if(!empty($email)){ echo $email;} ?>">
      
      <h5 class="title"><span class="badge badge-noreq">任意</span>電話番号<span class="err-area"><?php if(!empty($err_msg['tel'])){ echo $err_msg['tel'];} ?></span></h5>
      <input type="text" name="tel" class="long-text <?PHP if(!empty($err_msg['tel'])){ echo 'err';} ?>" value="<?php if(!empty($tel)){ echo $tel;} ?>">
